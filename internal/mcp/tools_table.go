...
func (s *Server) registerTableTools() {
	s.impl.AddTool(mcp.NewTool("memory_create_table_note",
		mcp.WithDescription("Create a CSV table note (ADR-016): reference (or upload) a CSV file AND create the markdown note that points to it, in one call. The note is a normal .md whose frontmatter declares `type: table` + `media: <attachment>.csv`, so it participates in tags/links/backlinks/graph/FTS like any note, and the SPA renders the CSV as a paginated table. Use it for long tabular data (audit reports, exports) that would bloat a markdown body — the report note then wikilinks the table note. The column headers and row count are inlined into the body for search; cell VALUES are not indexed, so WRITE A CAPTION saying what the table contains. Requires the table_notes feature enabled (GOSIDIAN_VAULT_TABLE_NOTES). Provide the CSV ONE way (cheapest first): `attachment` = path of a CSV already uploaded via the HTTP upload endpoint (POST to your MCP /sse URL with /sse replaced by /upload; bytes over HTTP, no token cost); `bridge_filename` (staged file, co-located); `source_path`; or base64 `data` (~1 token/char, last resort)."),
